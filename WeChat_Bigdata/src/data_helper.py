...
def create_dataloaders(args):
    ...
    folds = KFold(n_splits=args.KFold,shuffle=True,random_state=7777)
    data_ = np.arange(0,size)
    for train_index,val_index in folds.split(data_):
...
class MultiModalDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        asr_input, asr_mask = self.tokenize_text(text)
        text=''
        for o in self.anns[idx]['ocr']:
            text += o['text']
        ocr_input, ocr_mask = self.tokenize_text(text)


        text_input = torch.cat([title_input,asr_input,ocr_input])
        text_mask = torch.cat([title_mask,asr_mask,ocr_mask])
