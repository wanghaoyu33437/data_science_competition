        ...
        with autocast():
            if random.random() < args['mixup']:
                ...
                if args['RDrop']:
                    ...
                    loss = loss_A + loss_B + args['RDrop'] * kl_loss
                else:
                    loss = loss_A
            else:
...
if __name__ == '__main__':
    ...
    params = f'{args["name"]}_Size{args["input_size"]}_bs{args["batch_size"]}_Split2_ema{args["ema_decay"]}_step{args["ema_step"]}_{"Mixup"if args["mixup"] else ""}_LS{args["LS"]}{"_adv" if args["adv_train"] else ""}{"_RDrop" if args["RDrop"] else ""}/'
