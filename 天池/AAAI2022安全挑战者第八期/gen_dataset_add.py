...
modelnames=['GoogLeNet'] #,'GoogLeNet','SENet18',,'ResNet18''MobileNetV2
...
# #
# images = list(np.load('best_pgd8_test2w_ensemble/data.npy'))
#
# hard_labels = list(np.load('best_pgd8_test2w_ensemble/label.npy'))
# total=0
# num = [0]*10
...
for inputs,target in tqdm(test_batches):
    ...
    # SmoothingLabel = labelSmoothing(target,0.1)
    ...
    target = target.cuda()
    # 对抗样本
    delta = attack_pgd(models, inputs, target, norm='l_inf', epsilon=(8.0/255), alpha=pgd_alpha, attack_iters=10)
...
print(images.shape, images.dtype, one_hots.shape,one_hots.dtype,hard_labels.shape, hard_labels.dtype)
current_path = 'ADVImage'

if not os.path.exists(current_path):
    os.mkdir(current_path)
np.save(os.path.join(current_path,'data_2_.npy'), images)
# np.save(os.path.join(current_path,'label.npy'), one_hots)
# np.save(os.path.join(current_path,'label_sm01.npy'), hard_labels)