...
class GIID_Model(nn.Module):
    def __init__(self):
        ...
        self.lr = 1e-4
        self.networks = SCSEUnet(backbone_arch='seresnext50')
...
class ForgeryForensics():
    def __init__(self, fold=1):
        ...
        self.test_num = 1
        self.batch_size =8
        self.train_npy = 'fold_%d_train.npy' % fold
        self.val_npy = 'fold_%d_val.npy' % fold
        # if fold == 1:
        #     # Train num: 3000
        #     self.train_npy = 'fold_%d_train.npy'%fold
    ...
    def val(self):
        ...
        num =0
        for cnt, items in tqdm(enumerate(self.val_loader)):
            Ii, Mg = (item.cuda() for item in items[:-1])
            filename = items[-1]
            Mo, gen_loss = self.giid_model.process(Ii, Mg)
            gen_losses.append(gen_loss.item())
            ...
            for i in range(len(Mo)):
                cv2.imwrite('../user_data/res/val_fold%d_'%self.fold + gpu_ids[0] + '/' + filename[i][:-4] + '.png', Mo[i][..., ::-1])
        val_list = merge(val_npy=self.val_file)
        path_pre = '../user_data/res/val_merge_fold%d_'%self.fold + gpu_ids[0] + '/'
...
def merge(val_npy='val_1o5',fold=1):
    path = '/home2/WHY/04_FakeImageDetection/baseline/data/train/img/'
    path_d = '../user_data/res/val_fold%d_'%fold + gpu_ids[0] + '/'
    path_r = '../user_data/res/val_merge_fold%d_'%fold + gpu_ids[0] + '/'
    rm_and_make_dir(path_r)
    size_list = [512, 768, 1024]
    val_lists = val_npy
    name = []
    for p, _ in val_lists[:498*2]:
        if p.endswith('.png'):
            n = p.split('/')[-1].split('_')[0]
            name.append(n+'.jpg')
    val_lists=set(name)
    for file in val_lists:
        img = cv2.imread(path + file)
        H, W, _ = img.shape
        size_idx =0
        while size_idx < len(size_list) - 1:
